...
var (
	...
	blocksView        string // View type to filter blocks by (term, agents)
...
type BlockDetails struct {
	...
	View        string              `json:"view"`        // Canonical view type (term, agents)
...
func init() {
	...
	blocksListCmd.Flags().StringVar(&blocksView, "view", "", "restrict to view type (term/terminal, agents)")
...
func blocksListRun(cmd *cobra.Command, args []string) error {
	if v := strings.TrimSpace(blocksView); v != "" {
		if !isKnownViewFilter(v) {
			return fmt.Errorf("unknown --view %q; try one of: term, agents", v)
		}
	}
...
func isKnownViewFilter(f string) bool {
	...
	case "term", "terminal", "shell", "console", "agents":
		return true
	default:
		return false
	}
