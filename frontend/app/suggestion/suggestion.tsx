...
function SuggestionControlNoResults({ children }: { children?: React.ReactNode }) {
    return (
        <div className="flex items-center justify-center min-h-[120px] p-4">
            {children ?? <span className="text-muted">No Suggestions</span>}
        </div>
    );
}
...
function SuggestionControlInner({
    ...
    return (
        <div
            className={clsx(
                "w-96 rounded-lg bg-modalbg shadow-lg border border-edge-mid z-[var(--zindex-typeahead-modal)] absolute",
        ...
        >
            <div className="p-2">
                <input
                    ...
                    className="w-full bg-surface text-foreground px-4 py-2 rounded-md border border-edge-mid focus:outline-none focus:border-accent placeholder-secondary"
                    placeholder={placeholderText}
                />
            </div>
            {fetched &&
                (suggestions.length > 0 ? (
                    <div ref={dropdownRef} className="max-h-96 overflow-y-auto divide-y divide-edge-mid">
