...
const COLOURS: SceneColours = { body: "rgb(95,116,224)", hot: "rgb(175,187,240)", marker: "rgb(224,114,108)" };
...
describe("drawSceneToCanvas", () => {
    it("strokes every segment in the scene, dropping none", () => {
        const ctx = stubCtx();
        const scene = sceneOf();
        drawSceneToCanvas(ctx, scene, SIZE, COLOURS);
        expect(ctx.stroke).toHaveBeenCalledTimes(scene.segments.length);
    });

    it("fills every polygon in the scene, dropping none", () => {
        const ctx = stubCtx();
        const scene = sceneOf({ posture: "escalation" });
        expect(scene.fills.length).toBeGreaterThan(0);
        drawSceneToCanvas(ctx, scene, SIZE, COLOURS);
        expect(ctx.fill).toHaveBeenCalledTimes(scene.fills.length);
        // closed, not left open: an unclosed fan leaves the last edge to the fill rule rather than the form
        expect(ctx.closePath).toHaveBeenCalledTimes(scene.fills.length);
    });

    it("opens one path per primitive, so a fill never joins the stroke before it", () => {
        const ctx = stubCtx();
        const scene = sceneOf();
        drawSceneToCanvas(ctx, scene, SIZE, COLOURS);
        expect(ctx.moveTo).toHaveBeenCalledTimes(scene.segments.length + scene.fills.length);
    });

    it("clears the frame before drawing, so frames do not accumulate", () => {
        const ctx = stubCtx();
        drawSceneToCanvas(ctx, sceneOf(), SIZE, COLOURS);
        expect(ctx.clearRect).toHaveBeenCalledWith(0, 0, SIZE, SIZE);
    });

    it("does not throw when the scene is empty", () => {
        const ctx = stubCtx();
        const empty: AvatarScene = { ...sceneOf(), segments: [], fills: [], extent: 0 };
        expect(() => drawSceneToCanvas(ctx, empty, SIZE, COLOURS)).not.toThrow();
    });
});
...
describe("blendTones", () => {
    ...
    it("returns hex, because the hot tone is derived from whatever this produces", () => {
        // lighten() parses hex only. An rgb() string here would silently flatten the lit core and the front
        // arcs into the body tone for the length of every register transition.
