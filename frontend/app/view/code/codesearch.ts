...
// Pure: shape a flat list of grep matches into what the results pane draws, and how the keyboard
// moves through it. git's order is kept rather than sorted — it walks the index, so its order is
// already the repository's own.
...
// -1 is the query input above the rows: ArrowDown leaves it for the first row, and ArrowUp from the
// first row returns to it.
export function stepRow(at: number, delta: 1 | -1, count: number): number {
    return Math.min(Math.max(at + delta, -1), count - 1);
}
