...
- **All motion** — every `motion`/`AnimatePresence` block stays as tuned in the motion cut (`docs/superpowers/specs/2026-06-19-agents-tab-answering-layout-motion-design.md`). Motion polish is a separate follow-up if anything reads off once the new colors land.
