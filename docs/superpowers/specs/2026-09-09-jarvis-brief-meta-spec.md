...
**Date:** 2026-09-09
**Status:** B1–B6 built. B3 wave 2 and all of B4/B5 are not yet live-verified, and the CDP suite has not been reconciled with the retired composition (see the tracking table). Design settled in the mockups. B4–B5 were built from this file directly; B6 has its own plan.
...
| B4 | Profile and run reconfigure | n/a | **Built** — `JarvisProfile`/`ProfileOverride` gain machine, parallelism and workerRoute; `Run` gains `PlanGatePending`, which `CreateRun` resolves from the profile and `DagSubmit` consumes; the launcher hydrates from the resolved profile with a draft-scoped manual override; new `SetRunSettingsCommand` writes pre-DAG settings on the run and post-DAG settings on the TaskGroup under the scheduler's own mutation lock; the Brief gains a header Profile modal and a run session sheet. Committed as `2a10d98d` (32 files, +3165/-61). Not live-verified. Two residuals: `resolveRunPlan` still ignores `DefaultMode` (pinned by `TestResolveRunPlanDefaultsToQuickRegardlessOfProfile` — the profile's shape reaches launches through the hydrated launcher control instead), and `Save as project defaults` stores the run's flattened lead route rather than re-deriving the profile's. |
