...
|---|---|---|---|---|
| S1 | Embedding foundation | [spec](2026-07-24-jarvis-s1-embedding-foundation-design.md) | shipped | Built — opt-in BYOK embedding foundation (pkg/jarvisembed): OpenAI-compatible Embedder seam + dedicated sqlite-vec derived-layer index (static-linked via `sqlite_vec.Auto()`, outside the vault) + hybrid lazy/warm Reconcile (content-hash diff, only-changed re-embed, prune, model-change rebuild) + section-level chunks + scope-filtered vec0 KNN (scope filter on the vec0 metadata column) + typed ErrEmbeddingsDisabled degradation. No consumer wired (S2 is first); adds jarvis:embed* config + secretstore key. Build wiring: vendored `pkg/jarvisembed/csrc/sqlite3.h` + global `CGO_CFLAGS -I…/csrc` (preserve `-O2`); S2 threads it into the Taskfile — see docs/deferred.md. |
| S2 | Semantic consumers (L3 + L4) | [spec](2026-07-24-jarvis-s2-semantic-consumers-design.md) | shipped | Built — L3 (semantic recall, `pkg/jarvisrecall`): a semantic candidate source appended to `selectSeeds`' deterministic seeds (bounded by `kSem`), degrading to L1/L2 when embeddings are off. L4 (semantic attribution, `pkg/jarvisattrib/semantic.go`): an orphan-gated, window-pre-filtered, content-hash-cached semantic edge producer fired inside `EdgesFor` only when L1–3 are silent — proposes a low-confidence (`weightLayer4`=0.2, "weak") `informing`/`provenance=semantic` edge on `cosine ≥ semThreshold`, reusing D's ticket self-correction + accept/detach/backfill unchanged. jarvisembed additions: public `Embed`, keyed `EmbedCached` cache (`attrib_vectors` table, model+hash tagged, wiped on model change) + `Cosine`. Taskfile `build:server:internal` now threads `CGO_CFLAGS -I…/csrc` so `wavesrv` links sqlite-vec. No RPC/WaveObj/migration/frontend/`task generate`. PLACEHOLDER tunables in docs/deferred.md; recency-aware seed merge + per-run L4 gating deferred |
| S3 | Proactive resurfacing | [spec](2026-07-24-jarvis-s3-proactive-resurfacing-design.md) | shipped | Built — at Run dispatch, an off-band non-fatal hook in `CreateRunCommand` matches the run's goal against past vault work and persists at most one "related prior work" suggestion to `run.Meta`. New pure-Go `pkg/jarvisproactive`: a two-stage gate — deterministic cosine pre-filter over S1's index (`cosThreshold` 0.82, self-exclusion, node dedupe, `shortlistMax` 5) → one capable-model relevance judgement that picks a single best or "none" (the model judges a fixed candidate set, never searches). A `"none"` sentinel is persisted too, so a re-view never recomputes. Embeddings off / provider or model failure ⇒ no card, never an error and never a slowed dispatch. Frontend (`view/agents/proactive.ts` + `proactiveviews.tsx`): one dismissible card on the run body, mounted below the run header; dismissal is optimistic (module atom) + persisted via `ObjectService.UpdateObjectMeta`. Delivery rides the run's existing `waveobj:update` — no RPC, wps event, WaveObj, migration, or `task generate`. Also fixes an upstream identity gap this depends on: `wstore.AppendRun` takes the run by value, so `CreateRunCommand` never saw its own `OID` and every capture dossier linked `[[run-]]`; the run now stamps `OID`/`ChannelOID` locally before capture, making self-exclusion real. PLACEHOLDER tunables in docs/deferred.md; other triggers, ranked lists, global feed, and click-to-open the cited node deferred |
| U1 | Presence C ("Spaces") | [spec](2026-07-24-jarvis-u1-spaces-design.md) | shipped | Built — app-bar Space chip + Ctrl+P "Focus on task" group scope the Agent roster + Channels to a task's attributed runs (via D's EdgesFor); filter + "Show all" escape hatch; needs-you never suppressed; `ListDossiers`/`ResolveSpaceScope` pure-read RPCs (`pkg/wshrpc`). No embeddings/model/WaveObj/migration. Sessions/Radar/Jarvis-recall-default deferred |
| U2 | Tasks surface | [spec](2026-07-24-jarvis-u2-tasks-surface-design.md) | shipped | Built — new Tasks nav surface (`frontend/app/view/jarvis/`): dossier list grouped Active/Paused/Done + detail with a read-only machine-maintained panel (acceptance/state/blockers/refs, lock affordance), read-only human Notes, and an append-only decisions log. Two human writes: append a decision (human-attributed via new A `CreateHuman` → user git commit; the refs-index link stays a Jarvis write) and set status (machine `SetStatus`, conflict-retried once) with a confirm on terminal transitions. Backend: A `CreateHuman`, B `AppendHumanDecision` + `Dossier.Notes` projection, 4 wshrpc commands (`GetDossier`/`ListTaskDossiers` read, `AppendDossierDecision`/`SetDossierStatus` write); U1 `listDossiers` refactored to a shared status-filtered core. Deep-link "Open dossier ↗" from the U1 Space chip preselects the active Space's dossier. No embeddings/model/WaveObj/migration. Deferred (docs/deferred.md): in-Wave Notes editing, non-reserved frontmatter editing, editing existing decisions, manual dossier creation, graph/semantic edge rendering, live push. |
| U3 | Graph surface | [spec](2026-07-24-jarvis-u3-graph-surface-design.md) | shipped | Built — new Graph nav surface (`frontend/app/view/jarvis/jarvisgraph*.tsx`) rendering the vault as a force-directed map: the whole-vault wikilink web (task/decision/memory, colored by kind, circles) is the base canvas, and clicking a task blooms its attributed Run nodes (amber squares) in on typed attribution edges. Edges encode three dimensions at once — `informing` dashed vs `confirmed` solid, confidence bucket → opacity+width, `provenance=semantic` (S2's L4) → a distinct hue — so a provisional inferred edge never reads like a canonical dispatch one. A bottom-right selection card names the node and lists a task's runs with their buckets. Backend is two pure-read commands over a new `wavevault.Retriever.Graph()` (whole-scope subgraph, no seed/BFS): `VaultGraph` (base map) + `ResolveDossierEdges` (per-dossier bloom, lazily resolved and cached client-side). Renderer is a fork of `memgraph.tsx` sharing the now-structurally-typed `memgraphlayout.ts` helpers (module-scope position/camera cache, warmup + framed settle, degree-ranked label reveal). Read-only: no editing/detach/model-call/embedding. Nav placement chosen next to Memory rather than the spec's after-Jarvis slot, to preserve every existing `Ctrl+1..8` chord (`bindings.ts` binds only `SURFACE_ORDER.slice(0, 8)`). No WaveObj/migration. Deferred: search/filter, a read rail, cross-surface nav, live push, whole-vault attribution (bloom is per-focused-task). |
