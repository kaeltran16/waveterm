...
5. Ctrl+P `"ask-jarvis"` lead group + handoff. **(done — Plan 4)** — the ask row's `run()` starts a real recall conversation and opens the Jarvis surface where the cited answer streams; the palette stays list-of-actions (decision 1: no inline palette streaming — the UI-brief's "compact cited-answer inline in the palette" was amended, as the palette has no inline-render capability and every existing dispatch surfaces-then-closes). This is also Plan 3's `@jarvis` reroute live trigger for the *recall* path.
6. Contextual entries + ambient fixtures on Run / Radar / Memory. **(done — Plan 4)** — "Ask Jarvis about this" opens Jarvis with the object attached (`SourceRef`) in an `attached` scope + a suggested prompt pre-filled (state 11 real). Real `[n]`/grounding-card navigation covers `channel`/`run`/`agent`; `memory`/`radar`/`decision`/`commit`/`task` no-op deliberately (decision 2: those need new per-object focus plumbing). Ambient attribution (task tags + relevant-decision cards) ships as a deterministic fixture provider (`fixtureAmbientProvider`) owned by `view/agents/` so it adds no `agents → jarvis` coupling (decision 3: PLACEHOLDER data until attribution engine D, recorded in `docs/deferred.md`; radar `SourceRef` oref = `radar:<finding.id>` per decision 4).

Steps 1–2 are the contract-pinning core; 3 makes recall real; 4 is the risky migration; 5–6 are the entry points. Steps 3 and 4 are independent and can proceed in parallel once 2 lands. **All six steps are complete (Plans 1–4); G is feature-complete pending merge.**
