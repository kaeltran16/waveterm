...
## Execution progress (2026-09-03)

**Phases 0-3 and Tasks 4.1/4.2/4.4: COMPLETE**, landed in commit `1a37f1e3`
(`feat(orchestrate): DAG observability digest + engine hardening`). See that commit message for the
digest contract, cleanup split, DagOverview surface, and the engine hardening the work exposed.

**Phase 4 (remainder), Phase 5, and Phase 6.1-6.2: COMPLETE, uncommitted.**

- 4.3 — `pkg/orchestrate/askevents.go`: `AskTarget`, `ResolveAskTarget`, `RecordAskLifecycle`, and the
  `AskClearReason*` constants. The answer boundary is centralized on a new `agentask.AnswerHook` fired
  inside `DeliverAnswer`, wired once in `main-server.go`, so a cockpit answer, a Gatekeeper answer and
  `wsh jarvis dag answer` all record one `child-answered` row carrying the registry's ask id (the
  duplicate append in `DagAnswerCommand` is gone). `child-ask-cleared` gained its first producer at the
  two clear boundaries in `wshserver_ask.go`, guarded so a repeat clear appends nothing.
- 4.5 — `timelinefilter.ts` (`filterEvents`, `ATTENTION_KINDS`, `eventClickTarget`, `timelineLayout`)
  and `timelinerail.tsx`, mounted in the DAG modal. `runeventstore.ts` gained an honest
  `RunEventsStatus` + `retryRunEvents` so the rail can never print "live" over a load that never
  returned. All 17 new kinds gained titles, tones and RUN-group membership in `runtimeline.ts`.
- 5.1/5.2/5.4 — stable `ControlEnvelope` + `NewControlEventID`, the three authoritative outcomes
  (`sent` / `failed:write` / `failed:unavailable`) with bounded errors, the idempotent
  `PiControlAckCommand`, and `buildControl` in the digest.
- 5.3 — the pi parser preserves the envelope, `controlAckArgs` builds the confirmation, and the
  watcher acks only a command its dispatcher actually accepted (via the new hidden `wsh jarvis dag ack`).
- 6.1 — pure render-decision helpers `healthView` / `nextStepView` / `freshCounts` / `controlWarning`
  and `workerActivityText`. Fixes two dishonest states the overview had: an unavailable digest painted
  in success green, and an unreachable worker rendered as a fabricated idle agent.
- 6.2 — `timelineLayout` at `TIMELINE_RAIL_MIN_PX` (1100) drives rail-vs-drawer from `dagmodal.tsx`;
  `aria-live` is on health/attention only; rows, filters and toggles are all real buttons.

**Task 6.3 is NOT done.** `scripts/cdp/orchestrator-observability-e2e.mjs` is written (spec 10.3
items 1-8, syntax-checked) but has never been RUN: it needs `task dev` on the CDP port plus a live
orchestrator run with a DAG, and per this plan's own note, explicit approval for engine-managed
commits in a temporary registered git project. Nothing may claim it passes until it does.

**Decisions / deviations (flagged for review):**
- The rail is a NEW panel sharing `runtimeline.ts`'s projection; the run body's grouped collapsible
  `RunTimeline` is unchanged. Plan T4.5 step 3 reads as if `runtimelineview.tsx` itself becomes the
  rail, but spec 6.3 places the rail on the DAG/run layout and step 4 keeps `runtimelineview.tsx` as
  the run body's wrapper — turning the run body's grouped view into a flat list would have been a
  behavior change, not the "behavior preserved" the step asks for.
- The rail's worker-targeted rows SELECT the task rather than navigating: selection surfaces the graph
  rail's existing "Open in Agent", and spec 6.2 forbids a second navigation mechanism. The
  `onOpenWorker` prop this would have needed was therefore dropped rather than left unused.
- `lead-control-*` rows store `cmd` (the watcher command name) rather than the dag event kind, because
  that is exactly the enum `ControlDigest.Kind` is specified to carry.
- An unreachable lead now records `lead-control-failed` with kind `unavailable`, where `1a37f1e3`
  treated it as silent. Spec 7.1 makes it one of the three authoritative outcomes and the digest has
  an `unavailable` status to render it.
...
- [x] **Step 1: Add the contract types** verbatim to `wshrpctypes_dag.go` (the block above). Change `DagStatusCommand` return to `(*CommandDagStatusRtnData, error)`.
- [x] **Step 2: Run `task generate`.** Verify generated `wshclient.go` `DagStatusCommand` returns `*wshrpc.CommandDagStatusRtnData` and `frontend/app/store/wshclientapi.ts`/`frontend/types/gotypes.d.ts` carry the new types.
- [x] **Step 3: Typecheck both sides.** `go vet ./pkg/wshrpc/` and `node --stack-size=4000 node_modules/typescript/lib/tsc.js --noEmit -p tsconfig.json`. Expected: Go compiles (handler still returns `*TaskGroup`? — that breaks; see T1.3); TS clean (FE currently ignores return shape or uses `.Tasks` — fix in T3.1, keep tsc green here only if trivial; otherwise defer FE consumption to T3.1 and accept an intentionally typed cast in the interim — no: if tsc is red, land a minimal FE type update here: `dagstore.ts` status calls cast to the new return and read `.group`).
- [x] **Step 4: Checkpoint.** `task build:backend`, tsc green. Show diff.
...
- [x] **Step 1: Write failing tests for health precedence.** Copy the exact semantics of spec §5.2. Fixtures via `digest_test_helpers_test.go` (builder funcs returning a `TaskGroup` with N tasks in chosen states, a fake child `Run`, retained `RunEvent` rows, asks):
...
- [x] **Step 2: Run to verify they fail** (`go test ./pkg/orchestrate/ -run 'TestHealth'`).
- [x] **Step 3: Implement `buildCounts` + `buildHealth`.** Use the scheduler helpers; `RecoveredRetry` counts `done` tasks with a retained `task-retried` event for the same task id (spec §5.1); `MergeReady` counts done + (released-when-gated) + unmerged in merge-required DAGs; `Attention` per spec §5.1 (asks, unreleased gates, terminal failures, blocked merges, failed cleanup — not ordinary stalls or cleanup in progress).
- [x] **Step 4: Verify pass.** `go test ./pkg/orchestrate/ -run 'TestHealth|Test.*Digest'`. Expected: PASS.
- [x] **Step 5: Next-step tests.** Spec §5.3 ordering (answer → approve/sendback → resolve-merge → retry-cleanup → retry/skip/escalate → merge-ready → dispatch → parallelism-wait → dependency-wait → terminal). Assert `TaskIds` in DAG order within one condition, `BlockingTaskIds` populated for dependency waits, `Actions` complete and valid for the condition.
- [x] **Step 6: Implement `buildNext` + `buildTaskDigest`.** `WaitReason` per task; `FreshnessTs` from `TaskNode.LastActivity`; `CleanupState`/`MergeState` per spec §5.1 (cleanup `clear` whenever no debt, including before integration and after completion; `pending` while no error; `failed` when debt carries an error).
- [x] **Step 7: Duration tests** — `RunMs` = Σ complete child phase `[StartedTs, DoneTs]`; `MergeWaitMs` = `task-done`→first `task-merge-started`; `CleanupMs` = `task-cleanup-pending`→completed/failed; missing boundary → `Partial:true` and zero duration, never fabricated; elapsed = `Now-CreatedTs` while active, terminal timestamp minus `CreatedTs` after `dag-done`/`dag-cancelled`; a pruned required boundary marks the aggregate `Partial`.
- [x] **Step 8: Implement `buildDurations`.** Run child phase time-stamp fields live on `waveobj.RunPhase` (read `pkg/waveobj/wtype.go` Run/RunPhase first).
- [x] **Step 9: Version + stability tests** — `Digest.DagVersion == Group.Version`; same version with changed ask state → different `Next`/task `AskId` (spec §10.1: "same DAG version with changed ask state produces a new digest"); recoverd-retry derivation uses retained history + current success.
- [x] **Step 10: Full phase test + checkpoint.** `go test ./pkg/orchestrate/ -run 'Digest|Health'` green. Show diff.
...
- [x] **Step 1: `wstore_runevent_test.go` fails for the new query** (assert it returns only requested kinds, bounded limit, newest-first).
- [x] **Step 2: Implement `QueryRunEventsByKind`** (filter on `QueryRunEvents` output or a dedicated sql query — read `wstore_runevent.go` first; pick the smaller change).
- [x] **Step 3: Rewire DagStatus** to assemble the snapshot and call `BuildDigest`. `DagStatusCommand` now returns `{Group, Digest}`. Keep `DagActionCommand`/`DagMergeCommand` unchanged.
- [x] **Step 4: Handler test.** An existing dag test's status assertions are updated to read `.Group`; add an assertion that `Digest.Counts.Total == len(Group.Tasks)`.
- [x] **Step 5: Checkpoint.** `go test ./pkg/wshrpc/wshserver/ ./pkg/wstore/`, `task build:backend`, `task generate` if the type moved (it did not in this task). Show diff.
...
- [x] **Step 1: Failing test** — CLI status output for a needs-you digest shows the digest action set for the task, not a locally reconstructed alternative.
- [x] **Step 2: Run to verify it fails.**
- [x] **Step 3: Implement** — replace the CLI-local action derivation with the digest fields.
- [x] **Step 4: Checkpoint.** `go test ./cmd/wsh/...`, `task build:backend`. Show diff. **Phase 1 complete: shared digest is the single contract for CLI and UI.**
...
- [x] **Step 1: Failing test** covering: run missing → unresolved (not-dispatched); run with no phase → unresolved; run with tab phase → resolved TabId; run whose recorded phase has no tab oref but a run exists → unresolved with worker-session-unavailable reason.
- [x] **Step 2: Run to verify it fails.**
- [x] **Step 3: Implement** (`ResolveTaskWorker`; reuse `ResolveRunWorker`'s phase-walk pattern — do not copy tab-footgun logic, prefer the existing walk).
- [x] **Step 4: Verify pass + checkpoint.**
...
- [x] **Step 1: Failing test** — dispatched-with-tab, pending, missing-session, closed-session cases; `openTaskWorker` never called for pending.
- [x] **Step 2-4: TDD cycle.** Implement; run; pass.
...
- [x] **Step 1: Extract `StatusLine` + `ActivityLine` as pure presentational components** with exports matching the row's current markup (colors from `@theme` tokens, status always text-or-icon, never color alone).
- [x] **Step 2: Rewire `AgentRow`** to compose them. No test for the row (rendering); rely on `task verify:ui` (Phase 6.3).
- [x] **Step 3: tsc + lint checkpoint.** `node --stack-size=4000 ...tsc --noEmit -p tsconfig.json` and `npx eslint frontend/app/view/agents/`. Show diff.
...
- [x] **Step 1: Failing tests** — stale version rejected; same-version older-response-loses (ask-changes-during-load race, spec §10.2); refresh triggers from events; loading/error states.
- [x] **Step 2: Run to verify they fail.**
- [x] **Step 3: Implement** (pure functions + a thin hook over the existing wshclient call used by `dagstore.ts` today).
- [x] **Step 4: Verify pass + checkpoint.**
...
- [x] **Step 1: `workertasksort.test.ts` fails** — ordering per spec §6.1 buckets.
- [x] **Step 2: Implement `workertasksort.ts` + `nextStepText`** (pure).
- [x] **Step 3: Build `dagoverview.tsx`** composing health strip, next move, workers, queue; all text-or-icon status; `aria-live="polite"` only on health/attention transitions (spec §6.4).
- [x] **Step 4: Mount in `runbody.tsx`** above the lead transcript for orchestrator runs (`run.dagoref` present — reuse the same guard as the existing "Open DAG" button at runbody.tsx:~210).
- [x] **Step 5: tsc + tests checkpoint.**
...
- [x] **Step 1: Extend `dagmodalstate.test.ts`** — pending tasks never expose Open in Agent; dispatched tasks resolve and route through `jumpToAgent`.
- [x] **Step 2: Run to verify it fails.**
- [x] **Step 3: Implement modal changes** — thread the model, render the rail states.
- [x] **Step 4: tsc + tests checkpoint.** Show diff.
...
- [x] **Step 1: Add constants + docs.** No behavior yet.
- [x] **Step 2: Checkpoint** — compile only (`go build ./pkg/waveobj/`); the FE gets the new kinds via `task generate` at the end of this phase.
...
- [x] **Step 1: Failing tests** in `engine_test.go` — each new kind appears exactly once at its boundary; a stubbed append failure does not fail the engine action.
- [x] **Step 2: Run to verify they fail.**
- [x] **Step 3: Implement writers** at the existing spawn/stall/retry-event sites (read `engine.go` lines ~200-320 where `RunEventKindTaskSpawned`/`TaskStalled`/`TaskRetried` are appended; the completion/cancel paths are nearby).
- [x] **Step 4: Verify pass + retry-event tests still green** (`retryevent_test.go`).
...
- [x] **Step 1: Failing tests** — raise appends `child-ask` with cap; answer appends `child-answered` preserving ask id; clear appends `child-ask-cleared` with reason; duplicate clear appends nothing; summary > cap is truncated to exactly the cap.
- [x] **Step 2: Run to verify they fail.**
- [x] **Step 3: Implement `askevents.go` + wire the three call sites** (ask raise in the DagAsk/child correlation path; deliver in `deliver.go`; clear in the ask-clear/dismiss path).
- [x] **Step 4: Verify pass + existing dagask tests green** (`wshserver_dagask_test.go`).
...
- [x] **Step 1: Failing tests** — each new kind written exactly once at its boundary; cleanup phases emit pending → completed/failed matching the persisted transitions; a failed `AppendRunEvent` never fails the merge or cleanup.
- [x] **Step 2-4: TDD cycle.**
- [x] **Step 5: Run `task generate` and typecheck TS** (new kinds now visible to the FE); `node --stack-size=4000 ... tsc --noEmit -p tsconfig.json` stays clean.
...
- [x] **Step 1: `timelinefilter.test.ts` fails** — filters, task-scoped, attention set, and cross-selection routing.
- [x] **Step 2: Implement pure helpers** (`filterEvents`, `eventClickTarget`).
- [x] **Step 3: Refactor `runtimelineview.tsx` into the shared rail** with wide/drawer layout hooks (drawer decision implemented in Phase 6; here the component takes `layout: "rail" | "drawer"` prop).
- [x] **Step 4: Mount the rail** in the DAG/run layout; keep `runtimelineview.tsx` as the run body's thin wrapper.
- [x] **Step 5: tsc + tests checkpoint.** Show diff.
...
- [x] **Step 1: Failing tests** — envelope contains the six fields in order; sent write succeeds → `lead-control-sent` row with event/session ids; missing control dir → `lead-control-failed` kind `unavailable`; write error → kind `write` + bounded error; append failure does not fail delivery.
- [x] **Step 2: Run to verify they fail.**
- [x] **Step 3: Implement** — extend `controlMessage`, thread `taskid` from the DAG event call sites, add the writers.
- [x] **Step 4: Verify pass + checkpoint.**
...
- [x] **Step 1: Failing tests** — ack for a real sent event appends acknowledged once; repeat ack appends nothing; unknown event rejects; mismatched session rejects; ack for a failed-delivery event rejects.
- [x] **Step 2: Run to verify they fail.**
- [x] **Step 3: Implement** — RPC type (then `task generate`), handler, writer.
- [x] **Step 4: Verify pass + checkpoint.**
...
- [x] **Step 1: Read `pi/extensions/waveterm-tools.ts`** control-watcher section (find where `content`/`cmd` are parsed and dispatched).
- [x] **Step 2: Implement** — field preservation + ack invocation after successful dispatch; failure to ack is logged, never blocks the command.
- [x] **Step 3: Run `task sync:piartifacts`** and diff the generated artifacts minimally.
- [x] **Step 4: Checkpoint** — `go build ./...` and `npx eslint pi/extensions/`.
...
- [x] **Step 1: Failing tests** — the four states; latest-attempt-wins; id mismatch never transfers ack.
- [x] **Step 2-4: TDD cycle + checkpoint** — `go test ./pkg/orchestrate/ ./pkg/wshrpc/wshserver/`, `task build:backend`.
...
- [x] **Step 1: Failing tests** — each degradation maps to the right rendered state (pure render-decision helpers in `dagdigest.ts`/`taskcorrelate.ts`).
- [x] **Step 2-4: TDD cycle** + full frontend suite.
- [x] **Step 5: Checkpoint** — `npx vitest run frontend/app/view/orchestrate frontend/app/view/agents`, tsc, eslint.
...
- [x] **Step 1: Pure drawer-decision test** — `timelineLayout(width)` returns `"rail" | "drawer"` at the repo's breakpoint(s).
- [x] **Step 2: Implement drawer + focus + aria-live** (leaf-only aging mirrors `NarrationTimeline`'s existing approach — read it first).
- [x] **Step 3: tsc + eslint + vitest checkpoint.**
...
- [x] **Step 1: Read the existing orchestrator E2E driver** (`scripts/cdp/orchestrator-e2e.mjs` from the 08-27 plan) and port its scaffolding.
- [x] **Step 2: Write the scenario** asserting items 1-8; run it via `task verify:ui -- orchestrator-observability` (or the driver's equivalent invocation).
...
- [x] **Step 1: Full verification** (except the live E2E, see T6.3). `task build:backend`; `go test ./pkg/...` (CGO_CFLAGS set); `npx vitest run`; `node --stack-size=4000 node_modules/typescript/lib/tsc.js --noEmit -p tsconfig.json`; `npx eslint .`; `npx prettier --check .`.
- [x] **Step 2: Self-review the diff.** No commented-out code, no debug statements, no generated-file hand-edits, no unrelated files staged (working-tree leftovers from before this feature stay out).
- [x] **Step 3: Simplify review of changed lines** (pi-simplify): naming, redundancy, dead code.
