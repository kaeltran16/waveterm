...
- **Wave Vault** (`pkg/wavevault`, `memroots`) — the durable-knowledge store Jarvis reads: `memroots` is the single registry of vault locations (`memory:vaultpath` is the root's source of truth), `wavevault` the typed retriever over its collections (`tasks`, `decisions`, `attachments`, …). The memory subsystem that also lived here — `memvault`, `memgarden`, `memdistill`, the Vault surface and the 17 `Memory*` RPCs — was removed; see `docs/deferred.md` for the recovery command.
