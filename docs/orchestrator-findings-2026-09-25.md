...
| merge label | A task waiting its turn to merge offers `merge`, the `wsh jarvis dag merge` command and what the graph view already calls it; `resolve-merge` stays for a conflicted merge or a failed Verify (`digestActionMerge`, `digest.go`). | `TestDigestFinishedLaneIsMergedAtItsTip`, `TestNextMergeReadyFlatDag` |
| merge title | `landTitle` drops the writing-plans template's " Implementation Plan" and clips the subject to 72 characters, so a one-line goal no longer becomes a 1,200-character subject. | `TestLandTitleNamesTheChangeOnOneLine` |
