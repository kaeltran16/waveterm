...
|---|---|---|---|
| Channel lifecycle — rename / delete / archive / notes | `deleteChannel` only — `renameChannel`, `archiveChannel`, `setChannelNotes` and their RPCs deleted in `84f366fb` (one-channel-per-project collapse) | no — one channel per project leaves nothing to rename, archive or annotate | closed — `84f366fb`; what it left orphaned is listed in `docs/orchestrator-guide.md` (What the backlog run left open) |
