...
| ~~No first-token deadline: a lead that dies on its first model call is indistinguishable from one that is thinking~~ **fixed 2026-09-04** | bug | M | flaws tracker F13 + R10 addendum. Two paths: an agent that **exits** non-zero without stamping a transcript is now reported instead of dropped at `onexit.go` (seconds, not 15 min), and a child that **hangs** before its first token ages from its spawn time against `FirstTokenDeadline`. A dead lead fails its running phase, so the run derives `blocked` instead of reading `executing` forever |
...
| ~~Merge gate has no liveness and no age~~ **fixed 2026-09-04** | bug | M | flaws tracker F16 + R10. A merge-ready task open past `MergeGateStaleAfter` (30 min) counts as attention, so health leaves `healthy` for `needs-you`. Aged from the retained task-done boundary; a gate whose event was pruned has no clock and is deliberately left alone. **Not done:** the age is not *rendered* anywhere — the CLI and UI show that it needs you, not how long it has sat |
